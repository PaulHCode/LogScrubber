...
$parentModule = 'LogScrubber'
if (-not $ModuleName) { $ModuleName = 'LogScrubber.Client' }
...
$publishRoot = Join-Path -Path $WorkingDirectory -ChildPath 'publish\LogScrubber'
...
$originalManifestData = Import-PowerShellDataFile -Path "$publishRoot\LogScrubber.psd1"
$prereqHash = @{
