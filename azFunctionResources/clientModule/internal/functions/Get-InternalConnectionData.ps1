...
#>
	...
	process
	{
		try { $uri = '{0}{1}' -f (Get-PSFConfigValue -FullName 'LogScrubber.Client.Uri' -NotNull), $FunctionName }
		catch { $PSCmdlet.ThrowTerminatingError($_) }
		$header = @{ }
		
		#region Authentication
		$unprotectedToken = Get-PSFConfigValue -FullName 'LogScrubber.Client.UnprotectedToken'
		$protectedToken = Get-PSFConfigValue -FullName 'LogScrubber.Client.ProtectedToken'
